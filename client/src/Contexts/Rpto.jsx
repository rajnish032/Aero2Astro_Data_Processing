...
const RPTODataProvider = ({ children }) => {
  ...
  const handleFileUpload = async (file) => {
    ...
    try {
      const response = await axios.post(`${requestUrl}/user/rpto/upload-users`, formData, {
        headers: { rptoauth },
        withCredentials: true,
      });
  ...
  // Fetch users when page or searchQuery changes
  useEffect(() => {
    fetchUsers();
  }, [fetchUsers]);
