...
import { GisProject } from "../models/gis/projects.model.js";
...
export const addNewProject = expressAsyncHandler(async (req, res) => {
  ...
  const newProj = await GisProject.create({
    userId: req.user._id,
    ...projectData,
  });
//   console.log(newProj);
...
export const deleteProj = expressAsyncHandler(async (req, res) => {
  ...
  try {
    const project = await GisProject.findById(projId);
    ...
    await GisProject.findByIdAndDelete(projId);
...
export const getAllProj = expressAsyncHandler(async (req, res) => {
  ...
  const allProj = await GisProject.find({ userId: _id });
...
export const getAllPublicProj = expressAsyncHandler(async (req, res) => {
  ...
  const allProj = await GisProject.find({ userId: user._id });
