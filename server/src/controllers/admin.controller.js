...
// handle approval
export const handleGisApproval = asyncHandler(async (req, res) => {
  const { id } = req.params;
  ...
  await sendEmail({
    ...
    text: `Congratulations! ${existedUser.fullName} You have been approved by Aero2astro for the gis partnership with us`,
    html: `<html lang="en">
<head>
...
<body>
    <div class="email-container">
        ...
        <div class="content">
            ...
            <p>
                We are glad to inform you that your application for the Gis  partnership with aero2astro has been approved.
            </p>
            
            <p>
                You can now manage your profile and portfolio/ equipment assets by logging in to the  Gis Portfolio management System with the same credentials for free.
            </p>
...
`,
  ...
  return res.status(200).json({ message: " Gis has been approved" });
});

// handle rejection
export const handleGisRejection = asyncHandler(async (req, res) => {
  const { id } = req.params;
  const existedUser = await User.findById(id);
  if (!existedUser) return res.status(404).json({ message: "User not exist" });
  if (existedUser.status === "rejected")
  ...
  await sendEmail({
    from: "Aero2Astro Tech <tech@aero2astro.com>",
    to: existedUser.email,
    subject: `Update on Your Equipment Gis Partnership Application`,
    text: `Dear ${existedUser.fullName}, we regret to inform you that your application for the gis equipment partnership with Aero2Astro was not approved at this time.`,
    html: `<html lang="en">
        <head>
            <style>
                body {
        ...
        <body>
            <div class="email-container">
                ...
                <div class="content">
                    ...
                    <p>
                        Thank you for your interest in partnering with Aero2Astro as a Gis. We truly appreciate the time and effort you invested in your application.
                    </p>
                    <p>
  ...
  return res.status(200).json({ message: "Gis has been Rejected" });
});
...
//suspension
export const handleGisSuspension = asyncHandler(async (req, res) => {
  const { id } = req.params;
  const existedUser = await User.findById(id);
  if (!existedUser) return res.status(404).json({ message: "User not exist" });
  ...
  return res.status(200).json({ message: "Gis has been Suspended" });
});

//suspension Removal
export const handleGisRevokeSuspension = asyncHandler(async (req, res) => {
  const { id } = req.params;
  const existedUser = await User.findById(id);
  if (!existedUser) return res.status(404).json({ message: "User not exist" });
  ...
  return res.status(200).json({ message: "Gis Suspension had been Revoked" });
});
...
export const sendEmails = asyncHandler(async (req, res) => {
  ...
  if (users.length === 0) {
    return res.status(200).json({ message: "No pending gis found." });
  }
  const userEmails = users.map((user) => user.email);
  await sendEmail({
    from: "Aero2Astro Tech <tech@aero2astro.com>",
    bcc: userEmails,
    subject: "Complete Application for Gis  Approval",
    html: ` <html lang="en">
    <head>
      <meta charset="UTF-8">
      <meta name="viewport" content="width=device-width, initial-scale=1.0">
      <title>Reminder: Complete Application for Gis  Approval</title>
      <style>
        body {
          font-family: Arial, sans-serif;
    ...
    <body>
      <div class="email-container">
        <div class="header">
          <h1>Reminder: Complete Application for Gis  Approval</h1>
        </div>
        <div class="content">
          <p>Dear Gis,</p>
  ...
  res.status(200).json({ message: `Emails sent to ${users.length} gis.` });
});

//invite RPTO
export const inviteRPTO = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const signupLink = `https://rpto.aero2astro.com/register?token=${token}`;

    await sendEmail({
      from: "Aero2Astro Tech <tech@aero2astro.com>",
      to: email,
      subject: "Invitation to Create RPTO Account",
      html: ` 
      <html lang="en">
      <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>RPTO Invitation</title>
        <style>
          body {
            font-family: Arial, sans-serif;
            background-color: #f9f9f9;
      ...
      <body>
        <div class="email-container">
          <div class="header">
            <h1>Invitation to Create RPTO Account</h1>
          </div>
          <div class="content">
            <p>Dear RPTO,</p>
            <p>You have been invited to create an account as a RPTO.</p>
    ...
    res.json({ message: "Invitation sent successfully!", signupLink });
  } catch (error) {
...
//get RPTO
export const getRptos = asyncHandler(async (req, res) => {
  try {
    const rptos = await Rpto.find().select("-password");

    if (!rptos.length) {
      return res
        .status(404)
        .json({ success: false, message: "No RPTOs found" });
    }

    res.status(200).json({ success: true, rptos: rptos });
  } catch (error) {
    res
      .status(500)
...
//get Rpto Detail
export const getRptoDetails = async (req, res) => {
  try {
    const { id } = req.params;
    const { page = 1, limit = 15,fullName } = req.query;
    ...
    const stats = {
      numberOfGis: await User.countDocuments({ rpto: id, role: "Gis" }),
    ...
    res.status(500).json({ error: "Failed to fetch RPTO users" });
  }
};
