...
import { GisLog } from "../models/gis/gisLogs.model.js";
...
export const addNewLog = expressAsyncHandler(async (req, res) => {
    ...
    const newLog = await GisLog.create({
        userId:req.user._id,
        ...data
    });
    console.log(newLog)
...
export const deleteLog = expressAsyncHandler(async (req, res) => {
    ...
    const data = await GisLog.findByIdAndDelete({ _id:id });
    if (!data)
...
export const getAllLog = expressAsyncHandler(async (req, res) => {
    ...
    const allLog = await GisLog.find({ userId: _id }).populate("project");
...
export const getPublicAllLog = expressAsyncHandler(async (req, res) => {
  ...
  if (!uniqueId){
   ...
   const allLog = await GisLog.find({ userId:user._id }).populate("project");
